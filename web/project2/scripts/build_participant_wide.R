...
build_participant_wide <- function() {
  files <- list.files(
    here::here("data", "raw"),
    pattern = "sub-.*\\.csv$",
    full.names = FALSE #debug: mismatch with import_and_process
  )
  ...
  #refactor: vectorized version using lapply of looping through the data. This is cleaner, and renamed rows so it's clearer, too 
  participant_rows <- lapply(files, import_and_process)
  #OLD: 
  ##rows <- list()
  ...
  out  <- do.call(rbind, participant_rows)
