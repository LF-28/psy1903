...
import_and_process <- function(file_name) {
 ...
 ##debug: if mistach between column names
   if ("trial_type" %in% names(df)) {
    names(df)[names(df) == "trial_type"] <- "trialType"
   }
  if ("expectedCategoryDisplayed" %in% names(df)) {
 ...
 #### Subset questionnaire vs. task -------------------------------------------
  ...
  questionnaire_df <- df[df$trialType == "Questionnaire", , drop = FALSE] #debug: changing "questionnaire" to "Questionnaire"
