...
summarize_behavior <- function(data, rt_min = 300, rt_max = 900) {
  ...
  #refactored so that it is not hard coded, and instead uses the column names. This is best indexing practice, and it makes the code more reproducible and readable 
  valid_data_rt <- data[data$rt >= rt_min & data$rt <= rt_max & data$correct == TRUE, ]
  valid_data_acc <- data[data$rt >= rt_min & data$rt <= rt_max, ] 
  #old: valid_data_rt <- data[data[ , 3] >= 300 & data[ , 3] <= 900 & data[ , 12] == TRUE, ]
  #old: valid_data_acc <- data[data[ , 3] >= 300 & data[ , 3] <= 900, ]
  
  ## Center each participant’s reaction times:
  ## subtract the participant’s mean RT so centered RTs average to zero
##refactor: delted the loop that centered RTs becuase it was never used downstream or referenced again. 
  ...
  grp_rt <- factor(
    test_data_rt$expectedCategoryAsDisplayed, #debug: changed expectedCategoryDisplayed to expectedCategoryAsDisplayed 
