#### Eliza Fried, October 28, 2025: Foundations Practice in R -------------------------------------
...
experiment_data <- data.frame(subject_id, rt, condition, congruent)
...
experiment_data[experiment_data$rt>500, "rt"] ##or experiment_data$rt[rt>500]
experiment_data[1:5, c("subject_id", "rt")]
experiment_data[4,]
experiment_data[,"condition"]
experiment_data$condition

fast_trials2 <- experiment_data[experiment_data$rt < 500,]
incongruent_trials <- experiment_data[experiment_data$congruent == FALSE,]
fast_incongruent <- experiment_data[experiment_data$congruent == FALSE & experiment_data$rt < 500,]
nrow(fast_trials2)
...
mean(experiment_data[experiment_data$condition == "control", "rt"])
mean(experiment_data[experiment_data$condition == "incongruent", "rt"], na.rm = TRUE)
congruency_effect <- 517.5 - 488
print(paste("The congruency effect was", congruency_effect, "milliseconds."))
mean_rt <- mean()
z_score <- (experiment_data$rt - mean(experiment_data$rt, na.rm = TRUE)) / sd(experiment_data$rt, na.rm = TRUE)
experiment_data$rt_z <- z_score
head(experiment_data)
fast <- ifelse(experiment_data$rt < 500, TRUE, FALSE)
experiment_data$fast <- fast
head(experiment_data)
new_row2 <- data.frame(subject_id = 11, rt = 470, congruent = TRUE, condition = "control", rt_z = NA, fast = TRUE)
experiment_data <- rbind(experiment_data, new_row2)
tail(experiment_data)
experiment_data$rt_z <- NULL
names(experiment_data)

cleaned_data <- experiment_data[experiment_data$rt > 300 & experiment_data$rt < 700 & !is.na(experiment_data$rt),]
nrow(cleaned_data)
mean(cleaned_data$rt)
